...
仓库身份与资料安全收口已经完成：`C:/Users/Administrator/Documents/清单怪兽app` 是主项目唯一
事实源并已推送 GitHub `origin/master`；`C:/Users/Administrator/Documents/craft-demo` 是受主项目文档
约束的 Craft 可编辑设计工作区，已有独立本地 Git 基线；迁移恢复出的旧副本已可逆隔离到带
`_ARCHIVE_DO_NOT_USE_` 前缀的目录。后续工作只从主项目与 Craft 两个活跃路径接手，可继续产品开发、
Craft 设计与设备 Beta。
...
| qa_inspector | QA-REPO-ARCHIVE-001 | 已裁撤 | 1 |
| qa_inspector | QA-REPO-ARCHIVE-001-CLOSE | 已裁撤 | 1 |
...
| T-REPO-SAFETY-001 | 将主项目未提交工作按价值分类并提交落盘 | Repo-Beta | 已完成（收口 QA 通过） | T-REPO-IDENTITY-001 |
...
| T-REPO-ARCHIVE-001 | 迁移唯一有效事实后隔离旧恢复副本 | Repo-Beta | 已完成（收口 QA 通过） | T-REPO-SAFETY-001、T-CRAFT-SAFETY-001 |
| T-REPO-QA-ARCHIVE-001 | 独立验收旧副本可逆隔离与内容完整性 | QA-REPO-ARCHIVE-001 | 已通过 | T-REPO-ARCHIVE-001 |
| T-REPO-ARCHIVE-001-CLOSE | 按 QA 通过裁决收口旧副本隔离记录 | Repo-Beta | 已完成：e5f90c6 已推送 | T-REPO-QA-ARCHIVE-001 |
| T-REPO-QA-ARCHIVE-001-CLOSE | 只读复核旧副本隔离完成态 | QA-REPO-ARCHIVE-001-CLOSE | 已通过 | T-REPO-ARCHIVE-001-CLOSE |
...
### T-REPO-QA-SAFETY-001-CLOSE

任务ID：T-REPO-QA-SAFETY-001-CLOSE

目标：只读确认主仓安全任务已按 QA 裁决正式完成，完成态记录、注册表和远端提交一致。

背景：Repo-Beta 已创建并推送收口提交 `da65ee3`，报告本地 HEAD 与 `origin/master` 一致，任务正文和 registry 均为 `completed`，工作树干净。PM 派单会产生一处新的 `PROJECT_BOARD.md` 差异。

验收标准：
1. 任务状态页正文为 `completed`，历史证据和 checkpoint 风险仍保留，下一接手点指向 Craft 快照且旧副本隔离继续等待。
2. registry 对应条目为 `completed`，协议正文和其他任务条目未被误改。
3. 收口提交 `da65ee3` 只含授权的 status、registry 和按现状暂存的 PM 看板；当前 HEAD 与 `origin/master` 跟踪引用一致。
4. 除 PM 新增的 `PROJECT_BOARD.md` 派单差异外，工作树无其他变化。
...
依赖：T-REPO-SAFETY-001-CLOSE。

### T-REPO-QA-SAFETY-001-CLOSE 裁决

QA 裁决为 pass、无范围违规。状态页与 registry 均为 `completed`，历史证据和 checkpoint 风险保留，下一接手点正确指向 Craft 快照；收口提交 `da65ee3ed23b2108aef9aab20f3d4ae64197815f` 仅含授权记录与 PM 看板，本地 HEAD 和 `origin/master` 跟踪引用一致。主仓安全任务正式完成。

### T-CRAFT-SAFETY-001

任务ID：T-CRAFT-SAFETY-001

目标：为 `C:/Users/Administrator/Documents/craft-demo` 建立独立、可恢复的本地 Git 基线快照，并把快照位置和恢复锚点登记回主项目交接体系。

背景：Craft 工作区当前无 `.git` 元数据，约有 300 个应保全文件、合计约 9.69 MB；`node_modules/`、`dist/`、`.playwright-cli/` 和日志不应纳入。当前显式存档 `.craft-saves/current-workspace.json` 合法，`savedAt=2026-08-10T02:02:43.276Z`，活动工作区为“清单怪兽V2.0”（ID `builtin-list-monster-echo`），当前画板为 `lme-page-today`。用户已明确授权建立 Craft Git 快照；本任务只建立本地仓库，不添加远端、不上传 Craft 仓库。

验收标准：
1. 开工先按 Craft `AGENTS.md` 读取并验证 `.craft-saves/current-workspace.json`，在任务记录中写明 `savedAt`、活动工作区名称/ID、当前画板 ID，禁止重写该存档。
2. 在 Craft 根目录初始化本地 Git 仓库，初始分支为 `master`；不得添加任何 remote，不得改动主项目或旧恢复副本的 Git 历史。
3. 只允许编辑 Craft 的 `.gitignore`，新增 `.playwright-cli/` 排除规则；既有 `node_modules`、`dist`、日志和编辑器缓存排除规则保持有效。
4. 基线快照纳入：`.craft-saves/**`、`.do-dev/**`、`src/**`、`public/**`、`scripts/**`、`docs/**`、`output/**`、`AGENTS.md`、`README.md`、项目配置与锁文件、`page-sample.json`、`screenshot.png`；除 `.gitignore` 外均按当前内容提交，不改写。
5. 快照不得纳入：`node_modules/**`、`dist/**`、`.playwright-cli/**`、日志、编辑器缓存或其他已忽略临时产物；不得删除这些磁盘内容。
6. 提交前运行并取得明确结果：`npm run lint`、`npm run build`、`npm run verify:workspace-save`、`npm run verify:list-monster-workspace`、`npm run verify:list-monster-pc-workspace`；任一失败则停止提交，在任务记录标记 `blocked`。
7. 验证通过后执行 `git diff --check`，创建 `[T-CRAFT-SAFETY-001] 建立 Craft 工作区基线快照` 提交；提交后 Craft `git status --short` 为空，`git remote -v` 无输出，并记录完整提交哈希、文件数、体积与排除项。
8. 在主项目创建 `.features/T-CRAFT-SAFETY-001/status.md` 并更新 `.features/_registry.md` 为 `ready_for_qa`，完整记录 Craft 路径、提交哈希、验证证据、风险和下一接手点。
9. 将主项目的任务记录、registry 与 PM 当前 `PROJECT_BOARD.md` 按现状提交并正常推送 `origin/master`；不得修改看板内容。主项目提交采用 `[T-CRAFT-SAFETY-001] 登记 Craft 本地快照`，最终本地 HEAD 与 `origin/master` 跟踪引用一致。

允许修改的文件范围：
- `C:/Users/Administrator/Documents/craft-demo/.gitignore`
- `C:/Users/Administrator/Documents/craft-demo/.git/**`
- Craft 测试生成且已忽略的 `dist/**` 或临时缓存
- 主项目 `.features/T-CRAFT-SAFETY-001/status.md`
- 主项目 `.features/_registry.md`
- 两仓 Git 索引与提交对象
- 允许暂存但禁止改写的 Craft 内容：`.craft-saves/**`、`.do-dev/**`、`src/**`、`public/**`、`scripts/**`、`docs/**`、`output/**`、`AGENTS.md`、`README.md`、项目配置/锁文件、`page-sample.json`、`screenshot.png`
- 允许暂存但禁止改写：主项目 `PROJECT_BOARD.md`

任务记录：主项目 `.features/T-CRAFT-SAFETY-001/status.md`
...
### T-REPO-ARCHIVE-001 实施结果

Repo-Beta 已完成精确目录移动：旧路径不存在，归档路径 `C:/Users/Administrator/Documents/Codex/Recovered_Old_PC/Documents/_ARCHIVE_DO_NOT_USE_清单怪兽app-c20254a-20260811` 存在；移动前后旧副本 HEAD、`git status`、4039 个文件及 8 项 SHA-256 一致。Craft 未变化；主仓登记提交 `f33b000` 已推送且工作树干净，任务记录为 `ready_for_qa`。

### T-REPO-QA-ARCHIVE-001

任务ID：T-REPO-QA-ARCHIVE-001

目标：只读独立验收旧恢复副本隔离是否精确、可逆、无数据损失，并确认两处活跃仓库未受影响。

背景：实施 Agent 报告旧路径已移动到精确归档名，HEAD/status/4039 文件/8 项哈希不变；主仓记录提交 `f33b000` 已推送。PM 派发 QA 会新增一处主仓看板差异。

验收标准：
1. 原路径 `.../Recovered_Old_PC/Documents/清单怪兽app` 不存在；精确归档路径存在，且两者父目录边界正确，没有其他同名/模糊目标被移动。
2. 归档内 `.git` 存在且可读，HEAD 为 `c20254a36645493f220461ae016826685e3488fc`；`git status --short` 保留原 4 个已修改文档和未跟踪 `.codex/`，没有提交、清理或还原迹象。
3. 只读复算文件数和 manifest 中 8 项 SHA-256，与隔离前记录完全一致；归档内容无新增、缺失或哈希漂移。
4. 任务状态页与 manifest 明确记录原/归档绝对路径、前后证据和可逆恢复方法；正文与 registry 均为 `ready_for_qa`。
5. 主仓提交 `f33b000` 仅含授权任务记录、manifest、registry 和 PM 看板；主仓 HEAD 与 `origin/master` 跟踪引用一致。除 PM 新派单的看板差异外主仓无其他变化。
6. Craft 仓库仍为 `master`/`c7cf805...`、无 remote、工作树干净，显式存档仍存在。
7. 输出结构化 JSON：`task_id`、`verdict`、逐项 `criteria`、`blocking_issues`、`scope_check`、`archive_integrity_check`、`summary`。

允许修改的文件范围：无；三处目录全程只读。

任务记录：QA 不创建或修改记录；裁决由 PM 写入 `PROJECT_BOARD.md`。

禁止事项：不得移动回原路径、编辑、暂存、提交、推送、清理、修复引用或改变任何仓库状态。

依赖：T-REPO-ARCHIVE-001。

### T-REPO-QA-ARCHIVE-001 裁决

QA 裁决为 pass、无范围违规。原路径已不可误入，精确归档路径存在；归档 `.git`、HEAD、未提交状态、4039 个文件和 8 项 SHA-256 与移动前一致；主仓与 Craft 均未受影响，恢复方法明确。允许完成态收口。

### T-REPO-ARCHIVE-001-CLOSE

任务ID：T-REPO-ARCHIVE-001-CLOSE

目标：依据 QA 通过裁决，将旧副本隔离任务记录与 registry 正式收口为 `completed` 并推送主仓最终状态。

背景：T-REPO-QA-ARCHIVE-001 已 pass、无范围违规；归档内容完整且可逆，两处活跃仓库健康。当前只有 PM 看板裁决与派单差异。

验收标准：
1. `.features/T-REPO-ARCHIVE-001/status.md` 正文改为 `completed`，保留全部路径、manifest、验证和恢复方法。
2. 下一接手点明确三项安全任务均已完成，无需再操作归档副本；后续 Agent 只使用主项目和 Craft 两个活跃路径。
3. `.features/_registry.md` 对应条目同步 `completed`，不改协议正文或其他任务。
4. 只修改 status 与 registry；`PROJECT_BOARD.md` 仅按 PM 当前内容暂存，不改写；不得操作归档、Craft 或产品内容。
5. 创建 `[T-REPO-ARCHIVE-001] 收口旧副本隔离任务` 提交并正常推送主仓 `master`；核对 HEAD 与 `origin/master` 跟踪引用一致、工作树干净。

允许修改的文件范围：
- `.features/T-REPO-ARCHIVE-001/status.md`
- `.features/_registry.md`
- 主仓 Git 索引与提交对象
- 允许暂存但禁止改写：`PROJECT_BOARD.md`
...
禁止事项：不得修改其他文件；不得移动、删除或修改归档副本；不得操作 Craft；不得重写历史或 force push。

依赖：T-REPO-QA-ARCHIVE-001 pass。

### T-REPO-QA-ARCHIVE-001-CLOSE

任务ID：T-REPO-QA-ARCHIVE-001-CLOSE

目标：只读确认旧副本隔离任务完成态、registry、归档位置与主仓远端收口一致。

背景：Repo-Beta 已推送收口提交 `e5f90c6`，报告状态页和 registry 均为 `completed`、主仓工作树干净。PM 派单会新增一处看板差异。

验收标准：
1. 状态页与 registry 均为 `completed`，路径、完整性证据和恢复方法保留；下一接手点明确三项安全任务全部完成。
2. 收口提交 `e5f90c6` 仅含授权记录与 PM 看板；主仓 HEAD 与 `origin/master` 跟踪引用一致。
3. 原旧路径仍不存在，精确归档路径仍存在且 HEAD/status/manifest 一致；Craft 快照仍健康。
4. 除 PM 新派单看板差异外，主仓与 Craft 无新增变化。
5. 输出结构化 JSON：`task_id`、`verdict`、逐项 `criteria`、`blocking_issues`、`scope_check`、`summary`。

允许修改的文件范围：无；全程只读。

禁止事项：不得修改、移动、提交、推送、清理或修复任何内容。

依赖：T-REPO-ARCHIVE-001-CLOSE。

### 三项仓库安全任务最终裁决

`T-REPO-SAFETY-001`、`T-CRAFT-SAFETY-001`、`T-REPO-ARCHIVE-001` 均已完成实施、独立 QA、返工（如有）、完成态收口与只读复核：

- 主仓所有有价值内容已分组提交并推送 GitHub `origin/master`，Flutter 定向测试 5/5 通过，禁入项为零。
- Craft 工作区已有本地 Git 基线 `c7cf805d03e80361f791ab1a60c43827a5105404`，262 文件、10,123,005 字节、无 remote，五项验证和完整性检查通过。
- 旧恢复副本已移动到 `C:/Users/Administrator/Documents/Codex/Recovered_Old_PC/Documents/_ARCHIVE_DO_NOT_USE_清单怪兽app-c20254a-20260811`；原路径不存在，归档 HEAD、状态、4039 文件及 8 项哈希均与移动前一致，可通过原样重命名恢复。
- 后续 Agent 只使用主项目与 Craft 两个活跃路径；归档目录仅用于追溯，不承接新任务、不写入新事实。
